...
|----------------|---------|-------------|
| **CLI Tool Only** | `uv tool install specify-cli --force --from git+https://github.com/deyil/spec-kit` | Get latest CLI features without touching project files |
...
```bash
uvx --from git+https://github.com/deyil/spec-kit specify init --here --ai copilot
```
