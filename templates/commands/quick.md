...
1. `/speckit.constitution` (only if missing)
2. `/speckit.specify <description>`
3. `/speckit.plan <tech stack>`
4. `/speckit.tasks`
5. `/speckit.checklist` (auto-selected domains)
6. `/speckit.analyze`
...
│                                                                         │
│  2. CONSTITUTION CHECK (if missing)                                     │
│     └── Auto-generate if /memory/constitution.md doesn't exist          │
│                                                                         │
│  3. EXECUTE WORKFLOW (no checkpoints)                                   │
│     └── Run specify, plan, tasks, and checklist commands                │
│                                                                         │
│  4. CRITICAL CLARIFICATION (0-2 questions only)                         │
...
1. **Check for existing constitution**:
   ...
   - If missing, auto-generate using `/speckit.constitution <auto-generated prompt>`.

2. **Auto-generate prompt from user input**:
   ...
   - Run `/speckit.constitution` without user confirmation (zero prompts).

### Step 3: Context Scan

1. **Scan all available context sources** (see **Context Sources** section above):
...
2. **Auto-Select Checklist Domains**:
   - Determine relevant domains (always include `requirements.md`, then add API, UX, Security, or Performance based on analysis).
   - AI assistant selects these domains automatically without user confirmation.

3. **Generate checklists** using fast-mode (zero scoping questions):
